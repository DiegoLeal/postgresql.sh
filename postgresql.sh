...
#
# Vídeo de instalação do GNU/Linux Ubuntu Server 20.04.x LTS: https://www.youtube.com/watch?v=zDdCrqNhIXI
# Vídeo de instalação do LAMP Server no GNU/Linux Ubuntu Server 20.04.x LTS: https://www.youtube.com/watch?v=6EFUu-I3u4s&t
#
...
#
# Verificando se o usuário é Root, Distribuição é >=20.04 e o Kernel é >=5.13 <IF MELHORADO)
# [ ] = teste de expressão, && = operador lógico AND, == comparação de string, exit 1 = A maioria dos erros comuns na execução
clear
if [ "$USUARIO" == "0" ] && [ "$UBUNTU" == "20.04" ] && [ "$KERNEL" == "5.13" ]
	then
		echo -e "O usuário é Root, continuando com o script..."
		echo -e "Distribuição é >= 20.04.x, continuando com o script..."
		echo -e "Kernel é >= 5.13, continuando com o script..."
		sleep 5
	else
		echo -e "Usuário não é Root ($USUARIO) ou Distribuição não é >=20.04.x ($UBUNTU) ou Kernel não é >=5.13 ($KERNEL)"
...
echo -n "Verificando as dependências do PgAdmin4, aguarde... "
	for name in apache2 php python3
	do
...
#
echo
echo -e "Instalação do PostgreSQL e PgAdmin4 no GNU/Linux Ubuntu Server 20.04.x"
